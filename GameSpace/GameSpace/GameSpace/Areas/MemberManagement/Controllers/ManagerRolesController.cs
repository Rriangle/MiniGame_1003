...
namespace GameSpace.Areas.MemberManagement.Controllers
{
	...
	public class ManagerRolesController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var data = await db.ManagerAccountRoles.AsNoTracking().ToListAsync();
			return View(data);
		}
		...
		{
			using var db = CreateContext();
			var item = await db.ManagerAccountRoles
				.AsNoTracking()
		...
		public async Task<IActionResult> Create([Bind("ManagerId,ManagerRoleId")] ManagerAccountRole model)
		{
			if (!ModelState.IsValid) return View(model);

			using var db = CreateContext();
			db.ManagerAccountRoles.Add(model);
		...
		{
			using var db = CreateContext();
			var item = await db.ManagerAccountRoles.FirstOrDefaultAsync(m => m.ManagerId == managerId && m.ManagerRoleId == managerRoleId);
			if (item == null) return NotFound();
			return View(item);
		}
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public async Task<IActionResult> Edit(int managerId, [Bind("ManagerId,ManagerRoleId")] ManagerAccountRole model)
		{
		...
		{
			using var db = CreateContext();
			var item = await db.ManagerAccountRoles.FindAsync(managerId, managerRoleId);
			if (item != null)
			{
				db.ManagerAccountRoles.Remove(item);
				await db.SaveChangesAsync();
			}
			return RedirectToAction(nameof(Index));
		}
